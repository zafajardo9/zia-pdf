...
export default function PdfToTextTool() {
  ...
  const [customFileName, setCustomFileName] = useState(`${BRAND.filePrefix}-extracted`)
    ...
    <NativeToolLayout title="PDF to Text" description="Extract text using fast scan or deep local OCR." actions={pdfData && !pdfData.isLocked && !extractedText && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-xl p-12 text-center hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-blue-50 dark:bg-blue-900/20 text-blue-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><FileText size={32} /></div>
        ...
        <div className="max-w-md mx-auto">
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-xl border border-gray-100 dark:border-white/5 text-center">
            ...
            <input type="password" value={unlockPassword} onChange={(e) => setUnlockPassword(e.target.value)} placeholder="Password" className="w-full bg-gray-50 dark:bg-black rounded-lg px-6 py-4 border border-transparent focus:border-blue-500 outline-none font-bold text-center mb-4" />
            ...
            {!extractedText ? (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setExtractionMode('text')} className={`p-4 rounded-lg border-2 transition-all flex flex-col items-center ${extractionMode === 'text' ? 'border-blue-500 bg-blue-50/50 dark:bg-blue-900/10' : 'border-gray-100 dark:border-white/5'}`}><Zap size={20} className={extractionMode === 'text' ? 'text-blue-500' : 'text-gray-400'} /><span className="font-semibold uppercase text-[10px] mt-1">Fast Scan</span></button>
                  <button 
                    onClick={() => !isOcrDisabled && setExtractionMode('ocr')} 
                    disabled={isOcrDisabled}
                    className={`p-4 rounded-lg border-2 transition-all flex flex-col items-center ${extractionMode === 'ocr' ? 'border-blue-500 bg-blue-50/50 dark:bg-blue-900/10' : 'border-gray-100 dark:border-white/5'} ${isOcrDisabled ? 'opacity-40 grayscale grayscale-mask' : ''}`}
                  >
                    <ScanSearch size={20} className={extractionMode === 'ocr' ? 'text-blue-500' : 'text-gray-400'} />
                    <span className="font-semibold uppercase text-[10px] mt-1">{isOcrDisabled ? 'No OCR' : 'Deep OCR'}</span>
                  </button>
                </div>
                {isProcessing && (
                  <div className="space-y-2"><div className="w-full bg-gray-100 dark:bg-zinc-800 h-2 rounded-full overflow-hidden shadow-inner"><div className="bg-blue-500 h-full transition-all" style={{ width: `${progress}%` }} /></div><p className="text-center text-[10px] font-semibold text-gray-400 uppercase tracking-widest animate-pulse px-1">Scanning Document...</p></div>
                )}
                {!isProcessing && (
              ...
              <div className="space-y-6">
                <textarea readOnly value={extractedText} className="w-full h-80 bg-gray-50 dark:bg-black border border-gray-100 dark:border-white/5 rounded-lg p-4 font-mono text-[10px] resize-none outline-none focus:border-blue-500 dark:text-gray-300 shadow-inner" />
                <div className="flex gap-3">
                  ...
                  <button onClick={handleDownload} className="flex-[2] bg-gray-900 dark:bg-white text-white dark:text-black p-4 rounded-lg font-semibold flex items-center justify-center gap-2 shadow-sm active:scale-95 transition-all"><Download size={18} /> {isNative ? 'Save .txt' : 'Download'}</button>
                </div>
                <button onClick={() => { setExtractedText(''); setProgress(0); setPdfData(null); }} className="w-full py-2 text-gray-400 uppercase font-semibold text-[10px] hover:text-blue-500 transition-colors">Close File</button>
              </div>
            )}
          </div>
        </div>
