...
export default function RotateTool() {
  ...
  const [customFileName, setCustomFileName] = useState(`${BRAND.filePrefix}-rotated`)
    ...
    <NativeToolLayout title="Rotate PDF" description="Tap individual pages to rotate 90 degrees." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-xl p-12 text-center hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-blue-50 dark:bg-blue-900/20 text-blue-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><RotateCw size={32} /></div>
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-gray-100 dark:border-white/5 flex items-center gap-6 shadow-sm">
            <div className="w-12 h-16 bg-gray-50 dark:bg-black rounded-xl overflow-hidden shrink-0 border border-gray-100 dark:border-zinc-800 flex items-center justify-center text-blue-500 shadow-inner">{pdfData.thumbnail ? <img src={pdfData.thumbnail} className="w-full h-full object-cover" /> : <RotateCw size={24} />}</div>
          ...
          <div className="bg-blue-500/5 dark:bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 flex items-center gap-4 animate-pulse">
            <div className="w-10 h-10 bg-blue-500 text-white rounded-full flex items-center justify-center shrink-0 shadow-sm shadow-blue-500/20">
              <RotateCw size={20} />
            </div>
            <div className="flex-1">
              <h4 className="text-sm font-semibold text-blue-500 uppercase tracking-tight leading-none mb-1">Visual Editor</h4>
              <p className="text-xs text-blue-500/70 font-bold">Tap any page thumbnail below to rotate it 90° clockwise.</p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-[60vh] overflow-y-auto p-1 scrollbar-hide">
              {Array.from({ length: pdfData.pageCount }).map((_, i) => {
                ...
                return (
                  <div key={pageNum} onClick={() => rotatePage(pageNum)} className="relative group cursor-pointer aspect-[3/4] rounded-xl overflow-hidden border-2 border-transparent hover:border-blue-500 transition-all bg-gray-50 dark:bg-black shadow-sm">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/5 transition-colors">
                      <div className="bg-white dark:bg-zinc-800 text-blue-500 p-2 rounded-full opacity-0 group-hover:opacity-100 shadow-sm scale-75 group-hover:scale-100 transition-all border border-gray-100 dark:border-white/5">
                        <RotateCw size={20} />
                      </div>
                    </div>
