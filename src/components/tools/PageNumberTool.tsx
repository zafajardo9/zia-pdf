...
export default function PageNumberTool() {
  ...
  const [customFileName, setCustomFileName] = useState(`${BRAND.filePrefix}-numbered`)
    ...
    <NativeToolLayout title="Page Numbers" description="Add custom numbering to your PDF automatically." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-xl p-12 text-center hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-blue-50 dark:bg-blue-900/20 text-blue-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><Hash size={32} /></div>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-zinc-900 p-8 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm space-y-6">
              {!downloadUrl ? (
                <>
                  <div>
                    <label className="block text-[10px] font-semibold uppercase text-gray-400 mb-3">Label Format</label>
                    <input type="text" value={format} onChange={(e) => setFormat(e.target.value)} className="w-full bg-gray-50 dark:bg-black rounded-xl px-4 py-3 border border-transparent focus:border-blue-500 outline-none font-bold text-sm dark:text-white" placeholder="Page {n} of {total}" />
                    <p className="text-[8px] text-gray-400 mt-2">Use <b>{'{n}'}</b> for page number and <b>{'{total}'}</b> for total pages.</p>
                  </div>
                  
                  <div>
                    <label className="block text-[10px] font-semibold uppercase text-gray-400 mb-3">Position</label>
                    <div className="grid grid-cols-3 gap-2">
                      {(['top-left', 'top-center', 'top-right', 'bottom-left', 'bottom-center', 'bottom-right'] as Position[]).map(pos => (
                        <button key={pos} onClick={() => setPosition(pos)} className={`py-3 px-1 rounded-xl text-[8px] font-semibold uppercase transition-all border ${position === pos ? 'bg-blue-500 text-white border-blue-500 shadow-sm shadow-blue-500/20' : 'bg-gray-50 dark:bg-black text-gray-400 border-gray-100 dark:border-zinc-800'}`}>
                          {pos.replace('-', ' ')}
                        </button>
                      ))}
                    </div>
